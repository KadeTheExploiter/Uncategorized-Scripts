...
local RunService = cloneref(game:FindFirstChildOfClass("RunService"))
...
--[[
local Root = Character:FindFirstChild("HumanoidRootPart") -- ffcoc_and_name(Character, "BasePart", "HumanoidRootPart") (they cause too much lag.... eww)
local MyRoot = Local.Character:FindFirstChild("HumanoidRootPart") --ffcoc_and_name(Local.Character, "BasePart", "HumanoidRootPart")]]
...
local function GetNearest()
	...
	for _, v in ipairs(Players:GetChildren()) do
		...
		if Character and MyChar and Character.Name ~= MyChar.Name and (Aimbot_TeamCheck or v.Team ~= Local.Team) then
			...
			if Root and MyRoot and Humanoid then
				...
				local HealthCheck = Humanoid.Health ~= 0

				if Aimbot_Method == "Mouse" then
					local Distance = (RootPosition - Mouse.Hit.p).Magnitude

					if HealthCheck and Distance < NearestDistance then
						NearestDistance = Distance
						NearestCharacter = Character
					end
				else
					local RootDistance = (MyRoot.Position - RootPosition).Magnitude

					if HealthCheck and RootDistance < NearestDistance then
...
while true do
	...
	if Aimbot_Enabled then
		...
		if NearestRig then
			...
			Camera.CFrame = cfn(Camera.CFrame.Position, PartLock.Position)	
		end
	end
